...
|-----------|------|---------|------|
| `GoogleAdsSearch` | `POST /v23/customers/{customerId}/googleAds:search` | `customerId`（路徑）、`developer-token`（Header）、`login-customer-id`（Header，MCC）、`query`（GAQL 字串） | 執行任意 GAQL 查詢，四種監控條件均使用此 Action，以不同 GAQL 字串指定所需欄位與資料範圍 |
...
| Custom Connector 驗證失敗 | OAuth 授權過期、OAuth 設定錯誤 | 整個流程停止執行，立即通知管理者重新建立 Connection 授權 | ✅ 是 |
...
| 4 | 在 Power Automate 中建立 Custom Connector Connection，點選授權完成 OAuth 授權流程 |
