...
系統每日定時觸發，透過自訂連接器（Custom Connector）串接 Google Ads API，自動取得各 Campaign、AdGroup、Keyword 的廣告成效資料，並依據預先定義的監控規則進行異常判斷。一旦偵測到異常，系統將自動彙整所有異常項目，產生格式化通知 Email 及 SEM 差勁關鍵字 CSV 附件，寄送至指定營運人員，實現無人值守的廣告健診流程。
...
**7. 產生 HTML Email 內容**
使用字串組合或 HTML 模板，將異常清單轉換為格式化的 Email 內文，包含摘要表格與處理建議。
...
**11. 執行錯誤通知**
在各關鍵步驟配置 Configure run after（錯誤後執行）邏輯，若發生例外，自動發送管理者告警。
...
**主旨：** `[Google Ads 異常監控] 2026/06/01 發現 3 組異常`

```
本次檢查時間：2026/06/01 09:03
監控帳戶數：3 個客戶
異常總數：3 組
...
客戶     Campaign              上線日        已上線時間   今日 Cost
客戶A    2026_NewProduct_TW    2026-05-28    4 天        NT$0

▸ 處理建議：請確認廣告帳戶狀態、Campaign 設定與付款方式是否正常。

━━━━━━━━━━━━━━━━━━━━━━━━━━━

【條件 2】廣告累積花費異常 — 共 1 組

客戶     Campaign              預期累積花費     實際累積花費     超標比例
客戶B    2026_Summer_Sale      NT$150,000      NT$168,500      +12.3%

▸ 處理建議：請確認是否有競標策略異動，或評估是否需調整每日預算上限。

━━━━━━━━━━━━━━━━━━━━━━━━━━━

【條件 3】CTR 成效異常 — 共 1 組

客戶     Campaign              今日 CTR   昨日 CTR   前前日 CTR  差異變化率
客戶A    2026_Brand_TW         2.1%       3.8%       3.5%        -142%

▸ 處理建議：CTR 出現明顯下滑趨勢，請確認廣告素材、出價策略、競爭對手動態。

━━━━━━━━━━━━━━━━━━━━━━━━━━━

【條件 4】SEM 差勁關鍵字 — 詳見附件 CSV

附件：SEM差勁關鍵字_20260601.csv（共 5 組關鍵字）

▸ 處理建議：請 SEM 人員評估暫停或調整出價策略。

━━━━━━━━━━━━━━━━━━━━━━━━━━━

此信件由系統自動產生，請勿回覆。
如有疑問請聯絡 系統管理員 admin@company.com
```
...
3. 在 **Send an email** Action 中，將 CSV 字串以 Base64 編碼後作為附件加入

> 附件檔名建議格式：`SEM差勁關鍵字_YYYYMMDD.csv`
...
*文件版本：v1.1*
*建立日期：2026/06/01*
*最後更新：2026/06/10（修正 Section 7.5 / 7.6 / 9.3，更新為實際採用的單一端點 GoogleAdsSearch 設計）*
*文件狀態：規劃草稿，待工程團隊確認後進入實作*
